...
if($pnvNum+$CDN>0){
	open(NVAR,"<$ARGV[0].nv.loc") || die "No such file: nv.loc";
	while(<NVAR>){
		chomp;
		@temp = split(/\t/);
...
if($SVN>0){
	open(SVAR,"<$ARGV[0].$svDB.pgsim.vcf") || die "No such file: $svDB.pgsim.vcf";
	while(<SVAR>){
		next if(/^#/);
		chomp;
		@temp = split(/\t/);
...
if($PVN>0){
	open(PVAR,"<$ARGV[0].$pvDB.pgsim.vcf") || die "No such file: $pvDB.pgsim.vcf";
	while(<PVAR>){
		next if(/^#/);
		chomp;
		@temp = split(/\t/);
...
while($pointers[0]||$pointers[1]||scalar(keys %NV)||scalar(keys %SV)||scalar(keys %PV)){
	...
	if($p==4){
		...
		$currentPOS = $temp[1]+length($temp[3])-1;
		...
		if(rand()<$NIDR){#indel
		#	$geom=gsl_ran_geometric($rng->raw(), $GEOM_PROB_INDEL);
		#	$geom=$NIDL if($geom>$NIDL);
			$geom=int(rand($NIDL));
			if($geom%3!=0 && rand()>$FSR && $vcfline[7] eq 'CDSNV'){
				$vcfline[0]='chr0';
			}else{
				if(rand()<0.5){#deletion
					$vcfline[3]=substr($ref{$currentCHR},$vcfline[1]-1,$geom+1);
...
sub coord_regist {
	...
	if($allvar_cache[0]==$currentCHR && $allvar_cache[1]<=$currentPOS){
		...
		while($pointers[1]){
			@temp = split(/\t/,$pointers[1]);
			$temp[0]=XYM_trans($temp[0]);
			if($temp[0] == $currentCHR && $temp[1]<=$currentPOS || $temp[0] == 24 && $Gender == "Female"){
				$pointers[1]=<VAR>;
			}else{
...
#	print STDERR "$common_cache[0]_$common_cache[1]\t$allvar_cache[0]_$allvar_cache[1]\t$pointers[2]\t$pointers[3]\t$pointers[4]\t$currentCHR\t$currentPOS\t$minCHR\t$minPOS\n";
